...
20.times do # creation 20 gossips
  project = Project.create(title: Faker::Friends.character, 
...
10.times do # creation 10 tags et relation entre tag et gossip
  tag = Tag.create(title: Faker::NewGirl.character,
  					user_id: rand((User.first.id)..(User.last.id)))
  projects_tags = tag.projects << [Project.find(rand((Project.first.id)..(Project.last.id))), Project.find(rand((Project.first.id)..(Project.last.id)))]
end
...
20.times do # creation 20 commentaires
  comment = Comment.create(content: Faker::HarryPotter.quote, 
  							user_id: rand((User.first.id)..(User.last.id)), 
  							gossip_id: rand((Project.first.id)..(Project.last.id)))
end
